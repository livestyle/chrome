...
define(function(require) {
	...
	var editorFiles = require('./editorFiles');
	...
	function updateModel(tab, model, callback) {
		model.set('editorFiles', editorFiles.get('files'));
	...
	editorFiles.on('change:files', function() {
		var files = editorFiles.get('files');
		eachModel(function(model, id) {
			model.set('editorFiles', files);
		});
	});

	editorFiles.connect(client);
