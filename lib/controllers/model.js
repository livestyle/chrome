...
define(function(require) {
	...
	function createModel(id, callback) {
		...
		storage.get(id, function(items) {
			...
			model.set('editorFiles', editorModel.get('userStylesheets'));
			userStylesheets.validate(model.get('userStylesheets'), function(stylesheets) {
		...
		active: function(callback) {
			chrome.tabs.query({highlighted: true, windowType: 'normal'}, function(tabs) {
				...
				var next = function() {
					...
					var tab = tabs.pop();
					createModel(idFromTab(tab), function(model) {
